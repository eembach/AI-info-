...
class NHLModel_v3_0_3:
    ...
    def _analyze_skaters(self, game: Game, lean: Dict[str, str], force_all: bool = False) -> List[Pick]:
        ...
        for p in game.key_players:
            # v3.2.1: Safety Gate for Injured Players
            if p.is_injured:
                continue

            # Analyze Form First (to get Season Avgs)
            form = form_analyzer.analyze_form(p, game.date)
            p.recent_form = form 
            
            # v3.2.1: Update Player Stats from Live Data (Full Power Profile)
            # v3.2.2: DAMPENED PROFILING
            # Early season stats are too volatile. We only update if sample size is decent (>5 games)
            # AND we weight it with the prior (avg_sog on object).
            if form.get("Season_SOG", 0) > 0:
                # Weighted Update: 70% Prior, 30% New (Stability)
            ...
            # Now that we have updated averages, the dynamic profiler will be accurate.
            ...
            if "Volume" in p.status_note or "Elite" in p.status_note:
                # v3.2.0: Dynamic Threshold based on Player Caliber
                target_sa = 31.0
                if p.avg_sog > 3.5: target_sa = 29.0 # Elite Volume Shooters create their own shots
                if is_efficiency_favored: target_sa = 28.5 # Efficiency Favorites dominate possession

                if opponent.avg_sog_against >= target_sa:
            ...
            if "Playmaker" in p.status_note or "Scorer" in p.status_note:
                # v3.1.1: Revert to Strict Points Logic
                # "Relaxed" gates caused failures on Points Overs in Barnburners.
                # We require "Heater" or "Green Light" specific conditions.
                ...
                if opponent.goals_against_per_game >= 3.4 and form["Trend"] == "Heater":
                    ...
                    # Stricter: Only if also trending or Elite
                    # v3.2.2: REMOVED "TRENDING" from Green Light. 
                    # Only TRUE ELITES get the Green Light on 1.5 Points.
                    if "Elite" in p.status_note:
                        green_light_1_5 = True
                elif is_favored_in_mismatch and "Playmaker" in p.status_note and "Elite" in p.status_note: # Strict
                    green_light_1_5 = True
                elif is_efficiency_boost and "Elite" in p.status_note: # Strict Elite Only
                    green_light_1_5 = True
                ...
                if green_light_1_5:
                    ...
                    # v3.2.0: SMART POINTS UNDERS
                    # We re-introduce points unders but with strict "System Trap" context.
                    # We do not fade pure talent (Elite/Heater) even in Traps.
                    # We FADE average players in Trap games.
                    
                    if "Elite" in p.status_note or p.avg_points > 0.9 or form["Trend"] == "Heater":
                        pass # Elite Immunity
                    elif ("System Trap" in scenarios or "Trap Game" in scenarios) and p.avg_points < 0.7:
                        # Fade non-elite players in Trap Games
                        picks.append(Pick(game.id, p.name, "Points", 1.5, "Under", 4, f"Trap Fade: {p.name} (<0.7 PPG) vs System Trap.", 14.0))
                    elif form["Trend"] == "Cold" and p.avg_points < 0.8:
                         picks.append(Pick(game.id, p.name, "Points", 1.5, "Under", 5, f"Cold Streak Fade: {p.name} struggling.", 18.0))
                    else:
                        pass
