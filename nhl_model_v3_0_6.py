...
class ScenarioAnalyzer:
    TRAP_TEAMS = ["LAK", "UTA", "SEA", "NYI", "MIN", "BOS", "STL"]
    PACE_TEAMS = ["COL", "NJD", "TOR", "EDM", "VGK", "VAN"]
    SUPPRESSION_SIEGE_TEAMS = ["CAR", "LAK"]
    ...
    def analyze_scenario(game: Game) -> Dict[str, Any]:
        ...
        # 1. System Logic (Team Identity)
        # System Trap
        is_trap_game = False
        
        # Suppression Siege Check
        if game.home_team.code in ScenarioAnalyzer.SUPPRESSION_SIEGE_TEAMS:
            scenarios.append("Suppression Siege (Home)")
            lean["side"] = game.home_team.code
        elif game.away_team.code in ScenarioAnalyzer.SUPPRESSION_SIEGE_TEAMS:
            scenarios.append("Suppression Siege (Away)")
            lean["side"] = game.away_team.code

        # Trap Logic
        if game.home_team.code in ScenarioAnalyzer.TRAP_TEAMS or game.away_team.code in ScenarioAnalyzer.TRAP_TEAMS:
             if game.home_team.code in ScenarioAnalyzer.TRAP_TEAMS and game.away_team.code in ScenarioAnalyzer.TRAP_TEAMS:
                 scenarios.append("System Trap (Lock)")
                 is_trap_game = True
                 lean["total"] = "Under"
             elif game.away_team.code not in ScenarioAnalyzer.PACE_TEAMS and game.home_team.code not in ScenarioAnalyzer.PACE_TEAMS:
                 if game.home_team.code in ScenarioAnalyzer.TRAP_TEAMS:
                     scenarios.append("System Trap (Soft)")
                 else:
                     scenarios.append("System Trap")
                     is_trap_game = True
                 lean["total"] = "Under"
...
class NHLModel_v3_0_3:
    ...
    def _analyze_saves(self, game: Game, lean: Dict[str, str]) -> List[Pick]:
        ...
        if "Suppression Siege (Home)" in scenarios:
        ...
        if "Efficiency Mismatch (Away)" in scenarios: # Away Offense is elite vs Home Defense
        ...
        # RECALIBRATED SIEGE THRESHOLD (Was 33.0, Now 36.0 for Over 28.5)
        ...
        if "Suppression Siege (Away)" in scenarios:
             picks.append(Pick(game.id, game.away_goalie.name, "Saves", 27.5, "Under", 5, f"Suppression Siege: {game.away_team.code} allows very few shots.", 18.0))

        # Efficiency Fade (Away Goalie)
        if "Efficiency Mismatch (Home)" in scenarios: # Home Offense is elite vs Away Defense
        ...
        if game.away_team.avg_sog_for < 26.0 or "System Trap" in scenarios:
            picks.append(Pick(game.id, game.home_goalie.name, "Saves", 26.5, "Under", min(4 + confidence_boost_under, 5), f"Volume Fade: {game.away_team.code} is low event/Trap.", 12.0))
            
        if game.home_team.avg_sog_for < 26.0 or "System Trap" in scenarios:
            picks.append(Pick(game.id, game.away_goalie.name, "Saves", 26.5, "Under", min(4 + confidence_boost_under, 5), f"Volume Fade: {game.home_team.code} is low event/Trap.", 12.0))
    ...
    def _analyze_skaters(self, game: Game, lean: Dict[str, str]) -> List[Pick]:
        ...
        for p in game.key_players:
            ...
            if "Volume" in p.status_note or "Elite" in p.status_note:
                 ...
                 if (is_suppression or is_trap) and not is_efficiency_favored and not is_barnburner:
                     ...
                     if p.avg_sog >= 3.0: 
                         ...
                         if is_trap:
                             # Trap Asymmetry Check
                             if p.team == game.away_team.code:
                                 # Visitor in a Trap: SEVERE FADE
                                 rationale += " + Trap Victim (Visitor). High Confidence Fade."
                                 confidence = 5 
                             else:
                                 # Home Trapper: SOFT FADE / NEUTRAL
                                 # We are more cautious fading the Trap Setter as they control pace
                                 rationale += " + Trap Setter Context (Caution)."
                                 confidence = 3 # Significant downgrade for Home team shooters (Kempe Risk)
            ...
            # --- Points Logic ---
            green_light_1_5 = False # Initialize variable early to avoid UnboundLocalError
            if "Playmaker" in p.status_note or "Scorer" in p.status_note:
                ...
                is_efficiency_boost = False
                if "Efficiency Mismatch (Home)" in scenarios and p.team == game.home_team.code: is_efficiency_boost = True
                if "Efficiency Mismatch (Away)" in scenarios and p.team == game.away_team.code: is_efficiency_boost = True
                ...
                if opponent.goals_against_per_game >= 3.4 and form["Trend"] == "Heater":
                    ...
                    green_light_1_5 = True
                elif is_efficiency_boost and form["Trend"] != "Cold":
                    green_light_1_5 = True
