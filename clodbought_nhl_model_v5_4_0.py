#!/usr/bin/env python3
"""
Clodbought NHL Player-Prop Model v5.4.0 (Shot-Volume Architecture + Star Premium)

v5.4.0 IMPROVEMENTS - Based on market inefficiency analysis:
- Added STAR_PREMIUM system: Stars have inflated lines due to public Over bias
- Increased pace weight from 40% to 45% (primary edge driver)
- Expanded player database to 100+ players
- Improved line estimation with star premium adjustment
- Added pace multiplier enhancement (1.5x effect in extreme games)
...
1. Team pace/tempo (SOG for/against per game) - 45% weight (increased)
2. Goaltending matchup (elite goalies slow pace) - 25% weight
3. Defensive systems (trap teams limit attempts) - 15% weight
...
v5.4.0 STAR PREMIUM INSIGHT:
- Books add premium to star player lines (public loves betting Overs on stars)
- Analysis showed stars with inflated lines hit 80% Unders in sample
- We now INCLUDE star Unders when they have sufficient premium
- Non-stars have tighter lines, require stronger environment signal
"""
...
class Config:
    ...
    # v5.4.0: Keep at 0.25, boost comes from star premium system
    CALIBRATION_SHRINKAGE = 0.25        # Shrink probabilities 25% toward 0.5

    # v5.4.0: Pace Multiplier Enhancement
    # In extreme pace games, multiply the environment effect
    PACE_MULTIPLIER_EXTREME = 1.5       # 50% stronger effect in extreme games
    EXTREME_LOW_SOG_THRESHOLD = 52      # Combined SOG below this = extreme low
    EXTREME_HIGH_SOG_THRESHOLD = 66     # Combined SOG above this = extreme high
...
class EnvironmentPredictor:
    ...
    def predict(cls, home: TeamProfile, away: TeamProfile) -> EnvironmentPrediction:
        ...
        # =================================================================
        # COMBINE SCORES - v5.4 SHOT-VOLUME FOCUSED + PACE ENHANCEMENT
        # =================================================================
        ...
        # v5.4: Increased pace weight to 45% based on market inefficiency analysis
        combined_score = (
            pace_score * 0.45 +           # INCREASED from 40% - most important
            goalie_score * 0.25 +         # Elite goalies slow pace
            situational_score * 0.15 +    # B2B, blowout potential
...
class PropGenerator:
    ...
    # =================================================================
    # v5.4.0: STAR PREMIUM SYSTEM
    # Books inflate lines on famous players due to public Over bias
    # Analysis showed 80% Under hit rate on stars with inflated lines
    # Premium = how much books add to the line (in SOG)
    # =================================================================
    STAR_PREMIUM = {
        # Tier 1: Superstars - Maximum public attention
        "Nathan MacKinnon": 0.5,     # Highest volume, most public action
        "Connor McDavid": 0.4,       # Best player, heavy public bias
        "Auston Matthews": 0.5,      # Goal scorer reputation
        "Leon Draisaitl": 0.4,       # Elite scorer
        "David Pastrnak": 0.4,       # Sniper reputation
        "Nikita Kucherov": 0.4,      # Elite shooter
        "Alex Ovechkin": 0.4,        # Goal scoring legend

        # Tier 2: Stars - High public attention
        "Kirill Kaprizov": 0.3,      # Rising star
        "Cale Makar": 0.3,           # Elite D-man
        "Sidney Crosby": 0.3,        # Legend status
        "Jack Hughes": 0.3,          # Young star hype
        "Connor Bedard": 0.3,        # Rookie phenom hype
        "Mikko Rantanen": 0.3,       # Elite winger
        "Artemi Panarin": 0.2,       # Star playmaker
        "Mitch Marner": 0.2,         # High profile

        # Tier 3: Quality players - Moderate public attention
        "Kyle Connor": 0.2,
        "Matthew Tkachuk": 0.2,
        "Brady Tkachuk": 0.2,
        "Filip Forsberg": 0.2,
        "Tage Thompson": 0.2,
        "William Nylander": 0.2,
        "Chris Kreider": 0.2,
        "Jason Robertson": 0.15,
        "Sam Reinhart": 0.15,
        "Cole Caufield": 0.15,
    }
    ...
    # v5.4.0: Adjusted to work WITH star premium system
    PLAYER_LINE_VALUE = {
        # =================================================================
        ...
        # v5.4.0 ADDITIONS - Expanded database
        # Secondary players - generally more predictable
        "Jake Guentzel": -0.05,       # Sniper type
        "Martin Necas": +0.05,        # Playmaker
        "Zach Hyman": -0.03,          # Net-front, shoots everything
        "Carter Verhaeghe": +0.05,    # Opportunistic
        "Tomas Hertl": +0.08,         # Pass-first center
        "Ivan Barbashev": +0.10,      # Physical, low shot rate
        "Tyler Seguin": +0.05,        # Declining shot rate
        "Vincent Trocheck": +0.08,    # Playmaker
        "Timo Meier": -0.05,          # Volume shooter
        "Brandon Hagel": +0.10,       # Energy player, low shots
        "Pavel Zacha": +0.12,         # Playmaker
        "Charlie Coyle": +0.10,       # Two-way center
        "Marco Rossi": +0.08,         # Young playmaker
        "Joel Eriksson Ek": +0.10,    # Defensive center
        "JJ Peterka": +0.05,          # Young, developing
        "Drake Batherson": +0.05,     # Balanced player
        "Kevin Fiala": -0.03,         # Shooter
        "Quinton Byfield": +0.08,     # Young, pass-first
        "Mason McTavish": +0.10,      # Young, physical
        "Jordan Eberle": +0.08,       # Declining
        "Andre Burakovsky": +0.05,    # Streaky
        "Yegor Sharangovich": +0.05,  # Developing
        "Juraj Slafkovsky": +0.10,    # Young, physical
        "Kirby Dach": +0.10,          # Playmaker
        "Bryan Rust": +0.03,          # Opportunistic
        "Rickard Rakell": +0.05,      # Secondary scorer
        "Brock Nelson": +0.05,        # Two-way
        "Kyle Palmieri": +0.05,       # Declining
        "Tom Wilson": +0.12,          # Physical, low shots
        "Pierre-Luc Dubois": +0.08,   # Big body, pass-first
        "Alex DeBrincat": -0.05,      # Sniper
        "Patrick Kane": +0.08,        # Aging, playmaker now
        "Owen Tippett": -0.03,        # Shooter
        "Sean Couturier": +0.12,      # Defensive center
        "Sean Monahan": +0.08,        # Playmaker
        "Adam Fantilli": +0.05,       # Young
        "Dmitri Voronkov": +0.08,     # Physical
        "Steven Stamkos": -0.05,      # Still shoots
        "Jonathan Marchessault": +0.03,  # Balanced
        "Pavel Buchnevich": +0.05,    # Playmaker
        "Brandon Saad": +0.10,        # Physical, low shots
        "Conor Garland": +0.05,       # Small, less shots
        "Mikael Granlund": +0.08,     # Playmaker
        "Tyler Toffoli": +0.03,       # Opportunistic
        "Barrett Hayton": +0.10,      # Developing
        "Lawson Crouse": +0.10,       # Physical
        "John Tavares": +0.05,        # Aging, less volume
        "Gabriel Vilardi": +0.05,     # Developing
        "Ryan Nugent-Hopkins": +0.08, # Playmaker
        "Casey Mittelstadt": +0.08,   # Playmaker
    }
    ...
    # v5.4: Further expansion to 3-4 players per team for better coverage
    KEY_PLAYERS = {
        "COL": [
            ...
            {"name": "Casey Mittelstadt", "sog": 2.3, "pts": 0.6, "toi": 17.0},
        ],
        "EDM": [
            ...
            {"name": "Ryan Nugent-Hopkins", "sog": 2.4, "pts": 0.7, "toi": 18.0},
        ],
        "TOR": [
            ...
            {"name": "John Tavares", "sog": 2.6, "pts": 0.7, "toi": 17.0},
        ],
        "WPG": [
            ...
            {"name": "Gabriel Vilardi", "sog": 2.5, "pts": 0.6, "toi": 16.0},
        ],
        "FLA": [
            ...
            {"name": "Carter Verhaeghe", "sog": 2.8, "pts": 0.7, "toi": 17.0},
        ],
        "VGK": [
            {"name": "Jack Eichel", "sog": 4.0, "pts": 1.1, "toi": 20.0},
            {"name": "Mark Stone", "sog": 2.5, "pts": 0.9, "toi": 19.0},
            {"name": "Ivan Barbashev", "sog": 2.3, "pts": 0.6, "toi": 16.0},
            {"name": "Tomas Hertl", "sog": 2.5, "pts": 0.6, "toi": 17.0},
        ],
        "DAL": [
            ...
            {"name": "Tyler Seguin", "sog": 2.7, "pts": 0.6, "toi": 16.0},
        ],
        "CAR": [
            ...
            {"name": "Martin Necas", "sog": 2.7, "pts": 0.8, "toi": 18.0},
        ],
        "NYR": [
            ...
            {"name": "Vincent Trocheck", "sog": 2.4, "pts": 0.6, "toi": 17.0},
        ],
        "NJD": [
            ...
            {"name": "Timo Meier", "sog": 2.9, "pts": 0.6, "toi": 17.0},
        ],
        "TBL": [
            {"name": "Nikita Kucherov", "sog": 3.5, "pts": 1.5, "toi": 20.0},
            {"name": "Brayden Point", "sog": 3.0, "pts": 0.9, "toi": 19.0},
            {"name": "Jake Guentzel", "sog": 2.9, "pts": 0.8, "toi": 18.0},
            {"name": "Brandon Hagel", "sog": 2.5, "pts": 0.6, "toi": 17.0},
        ],
        "BOS": [
            {"name": "David Pastrnak", "sog": 4.2, "pts": 1.2, "toi": 20.0},
            {"name": "Brad Marchand", "sog": 2.5, "pts": 0.8, "toi": 18.0},
            {"name": "Pavel Zacha", "sog": 2.3, "pts": 0.6, "toi": 17.0},
            {"name": "Charlie Coyle", "sog": 2.2, "pts": 0.5, "toi": 16.0},
        ],
        "MIN": [
            {"name": "Kirill Kaprizov", "sog": 3.5, "pts": 1.3, "toi": 21.0},
            {"name": "Matt Boldy", "sog": 2.5, "pts": 0.7, "toi": 17.0},
            {"name": "Marco Rossi", "sog": 2.3, "pts": 0.6, "toi": 16.0},
            {"name": "Joel Eriksson Ek", "sog": 2.4, "pts": 0.5, "toi": 18.0},
        ],
        "BUF": [
            ...
            {"name": "JJ Peterka", "sog": 2.3, "pts": 0.6, "toi": 16.0},
        ],
        "OTT": [
            ...
            {"name": "Drake Batherson", "sog": 2.5, "pts": 0.7, "toi": 17.0},
        ],
        "CHI": [
            {"name": "Connor Bedard", "sog": 3.5, "pts": 1.0, "toi": 19.0},
            {"name": "Philipp Kurashev", "sog": 2.2, "pts": 0.5, "toi": 16.0},
            {"name": "Tyler Bertuzzi", "sog": 2.4, "pts": 0.5, "toi": 17.0},
        ],
        "LAK": [
            {"name": "Adrian Kempe", "sog": 3.2, "pts": 0.8, "toi": 18.0},
            {"name": "Anze Kopitar", "sog": 2.2, "pts": 0.7, "toi": 19.0},
            {"name": "Kevin Fiala", "sog": 2.6, "pts": 0.7, "toi": 17.0},
            {"name": "Quinton Byfield", "sog": 2.3, "pts": 0.5, "toi": 16.0},
        ],
        "ANA": [
            ...
            {"name": "Mason McTavish", "sog": 2.3, "pts": 0.5, "toi": 16.0},
        ],
        "SEA": [
            {"name": "Jared McCann", "sog": 3.0, "pts": 0.9, "toi": 18.0},
            {"name": "Matty Beniers", "sog": 2.2, "pts": 0.6, "toi": 17.0},
            {"name": "Jordan Eberle", "sog": 2.4, "pts": 0.6, "toi": 16.0},
            {"name": "Andre Burakovsky", "sog": 2.3, "pts": 0.5, "toi": 15.0},
        ],
        "CGY": [
            {"name": "Nazem Kadri", "sog": 3.2, "pts": 0.9, "toi": 19.0},
            {"name": "Jonathan Huberdeau", "sog": 2.2, "pts": 0.7, "toi": 18.0},
            {"name": "Yegor Sharangovich", "sog": 2.5, "pts": 0.6, "toi": 16.0},
            {"name": "Mikael Backlund", "sog": 2.0, "pts": 0.4, "toi": 17.0},
        ],
        "MTL": [
            {"name": "Cole Caufield", "sog": 3.5, "pts": 0.9, "toi": 18.0},
            {"name": "Nick Suzuki", "sog": 2.5, "pts": 0.8, "toi": 19.0},
            {"name": "Juraj Slafkovsky", "sog": 2.3, "pts": 0.5, "toi": 16.0},
            {"name": "Kirby Dach", "sog": 2.2, "pts": 0.5, "toi": 16.0},
        ],
        "PIT": [
            {"name": "Sidney Crosby", "sog": 3.0, "pts": 1.0, "toi": 20.0},
            {"name": "Evgeni Malkin", "sog": 2.8, "pts": 0.8, "toi": 17.0},
            {"name": "Bryan Rust", "sog": 2.6, "pts": 0.6, "toi": 17.0},
            {"name": "Rickard Rakell", "sog": 2.4, "pts": 0.5, "toi": 16.0},
        ],
        "NYI": [
            {"name": "Bo Horvat", "sog": 3.0, "pts": 0.8, "toi": 18.0},
            {"name": "Mathew Barzal", "sog": 2.8, "pts": 0.9, "toi": 19.0},
            {"name": "Brock Nelson", "sog": 2.6, "pts": 0.7, "toi": 18.0},
            {"name": "Kyle Palmieri", "sog": 2.5, "pts": 0.5, "toi": 16.0},
        ],
        "WSH": [
            {"name": "Alex Ovechkin", "sog": 4.0, "pts": 0.9, "toi": 18.0},
            {"name": "Dylan Strome", "sog": 2.5, "pts": 0.8, "toi": 17.0},
            {"name": "Tom Wilson", "sog": 2.3, "pts": 0.5, "toi": 16.0},
            {"name": "Pierre-Luc Dubois", "sog": 2.4, "pts": 0.5, "toi": 17.0},
        ],
        "DET": [
            {"name": "Lucas Raymond", "sog": 2.8, "pts": 0.8, "toi": 18.0},
            {"name": "Dylan Larkin", "sog": 2.5, "pts": 0.7, "toi": 19.0},
            {"name": "Alex DeBrincat", "sog": 2.9, "pts": 0.7, "toi": 17.0},
            {"name": "Patrick Kane", "sog": 2.3, "pts": 0.6, "toi": 16.0},
        ],
        "PHI": [
            {"name": "Travis Konecny", "sog": 3.2, "pts": 0.9, "toi": 19.0},
            {"name": "Matvei Michkov", "sog": 2.8, "pts": 0.7, "toi": 17.0},
            {"name": "Owen Tippett", "sog": 2.6, "pts": 0.6, "toi": 16.0},
            {"name": "Sean Couturier", "sog": 2.2, "pts": 0.5, "toi": 17.0},
        ],
        "CBJ": [
            {"name": "Kirill Marchenko", "sog": 3.0, "pts": 0.7, "toi": 17.0},
            {"name": "Sean Monahan", "sog": 2.5, "pts": 0.6, "toi": 17.0},
            {"name": "Adam Fantilli", "sog": 2.3, "pts": 0.5, "toi": 16.0},
            {"name": "Dmitri Voronkov", "sog": 2.4, "pts": 0.5, "toi": 15.0},
        ],
        "NSH": [
            {"name": "Filip Forsberg", "sog": 3.5, "pts": 0.9, "toi": 19.0},
            {"name": "Roman Josi", "sog": 2.8, "pts": 0.8, "toi": 24.0},
            {"name": "Steven Stamkos", "sog": 2.9, "pts": 0.7, "toi": 17.0},
            {"name": "Jonathan Marchessault", "sog": 2.7, "pts": 0.6, "toi": 17.0},
        ],
        "STL": [
            {"name": "Jordan Kyrou", "sog": 3.0, "pts": 0.8, "toi": 18.0},
            {"name": "Robert Thomas", "sog": 2.2, "pts": 0.9, "toi": 19.0},
            {"name": "Pavel Buchnevich", "sog": 2.5, "pts": 0.7, "toi": 17.0},
            {"name": "Brandon Saad", "sog": 2.3, "pts": 0.5, "toi": 16.0},
        ],
        "VAN": [
            ...
            {"name": "Conor Garland", "sog": 2.4, "pts": 0.6, "toi": 16.0},
        ],
        "SJS": [
            {"name": "Macklin Celebrini", "sog": 2.5, "pts": 0.6, "toi": 17.0},
            {"name": "William Eklund", "sog": 2.2, "pts": 0.5, "toi": 16.0},
            {"name": "Mikael Granlund", "sog": 2.3, "pts": 0.6, "toi": 17.0},
            {"name": "Tyler Toffoli", "sog": 2.6, "pts": 0.5, "toi": 16.0},
        ],
        "UTA": [
            {"name": "Clayton Keller", "sog": 3.0, "pts": 0.8, "toi": 19.0},
            {"name": "Nick Schmaltz", "sog": 2.5, "pts": 0.7, "toi": 18.0},
            {"name": "Barrett Hayton", "sog": 2.2, "pts": 0.5, "toi": 16.0},
            {"name": "Lawson Crouse", "sog": 2.4, "pts": 0.5, "toi": 16.0},
        ],
    }
    ...
    def _create_sog_prop(
        ...
        v5.4: Added star premium system - stars with inflated lines are good Under bets
        """
        direction = env.sog_direction
        ...
        # v5.4: Get star premium (how much books inflate the line)
        star_premium = cls.STAR_PREMIUM.get(name, 0.0)
        is_star = star_premium >= 0.2

        # v5.4: New logic for star players
        # Stars with premium >= 0.3 are good Under bets BECAUSE their lines are inflated
        # This overrides negative player_value for stars
        if direction == PropDirection.UNDER:
            if star_premium >= 0.3:
                # Star with big premium - INCLUDE for Unders
                # The inflated line creates value even if player is a volume shooter
                pass
            elif player_value < -0.08 and star_premium < 0.2:
                # Non-star who beats their line - skip
                return None

        # Skip players with positive value for Overs (unless environment is extreme)
        ...
        # v5.4: Enhanced multiplier for extreme games
        env_multiplier = 1.0
        if env.environment == GameEnvironment.HIGH_EVENT:
            env_multiplier = 1.10  # +10% in high event
            if env.expected_total_sog >= Config.EXTREME_HIGH_SOG_THRESHOLD:
                env_multiplier = 1.15  # +15% in extreme high event
        elif env.environment == GameEnvironment.LOW_EVENT:
            env_multiplier = 0.90  # -10% in low event
            if env.expected_total_sog <= Config.EXTREME_LOW_SOG_THRESHOLD:
                env_multiplier = 0.85  # -15% in extreme low event

        expected_sog = avg_sog * env_multiplier

        # v5.4: Improved line estimation with star premium
        # Books add premium to star player lines
        base_line = round(avg_sog - 0.3, 0) + 0.5
        book_line = base_line + star_premium  # Stars get inflated lines

        # Calculate edge based on direction, incorporating player value AND star premium
        if direction == PropDirection.OVER:
            edge = expected_sog - book_line
            base_prob = 0.50 + edge / 3 - player_value
            probability = cls._calibrate_probability(base_prob)
        else:  # UNDER
            edge = book_line - expected_sog
            # v5.4: Star premium adds to Under probability
            # Stars with inflated lines = more room to go under
            star_boost = star_premium * 0.15  # 0.3 premium = +4.5% prob boost
            base_prob = 0.50 + edge / 3 + player_value + star_boost
            probability = cls._calibrate_probability(base_prob)
        ...
        value_note = ""
        if star_premium >= 0.3:
            value_note = f" [STAR PREMIUM +{star_premium:.1f}]"
        elif player_value > 0.10:
            value_note = f" [+value]"
...
def main():
    parser = argparse.ArgumentParser(description="Clodbought NHL Model v5.4.0 (Environment-First + Star Premium)")
