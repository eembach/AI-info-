...
class NHLModel_v2_5_7:
    ...
    def analyze_slate(self, games: List[Game]) -> List[Pick]:
        picks = []
        print(f"Running Model v2.6.0 (EV Edition) Analysis for {len(games)} games...")
        for game in games:
    ...
    def _analyze_skaters(self, game: Game, lean: Dict[str, str]) -> List[Pick]:
        ...
        for p in game.key_players:
            ...
            if "Playmaker" in p.status_note or "Scorer" in p.status_note:
                ...
                if opponent.goals_against_per_game >= threshold_ga or is_favored_in_mismatch:
                    confidence = 4
                    rationale_suffix = "."
                    
                    if form["L5_Points"] >= 1.5: 
                        confidence = 5
                        rationale_suffix = f" + HOT ({form['L5_Points']:.1f} PPG L5)."
                    elif lean["total"] == "Over":
                        rationale_suffix = " + Barnburner Context."
                    
                    # Boost Points over SOG in Blowouts for Elite Playmakers
                    if is_favored_in_mismatch and ("Playmaker" in p.status_note or "Elite" in p.status_note):
                        confidence = 5
                        rationale_suffix += " + Blowout Script (Points > SOG)."
...
def main():
    ...
    print("\n🏆 MASTER PICK BOARD (Model v2.6.0 - EV Adjusted) 🏆")
