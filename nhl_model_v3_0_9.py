...
class NHLModel_v3_0_3:
    ...
    def _analyze_saves(self, game: Game, lean: Dict[str, str]) -> List[Pick]:
        ...
        if game.away_team.avg_sog_for < 26.0 or "System Trap (Home)" in scenarios:
            line = 26.5
        ...
        if game.home_team.avg_sog_for < 26.0 or "System Trap (Away)" in scenarios:
            line = 26.5
            conf = min(4 + confidence_boost_under, 5)
            # Safety Margin Check
        ...
        for p in game.key_players:
            ...
            if "Volume" in p.status_note or "Elite" in p.status_note:
                 ...
                 if (is_suppression or is_trap) and not is_efficiency_favored and not is_barnburner:
                     ...
                     if p.avg_sog >= 3.0: 
                         ...
                         if is_trap:
                             # v3.0.10 Exception: Elite Player Immunity
                             # Elite players (Avg SOG > 4.0) are matchup-proof and should not be faded in Trap games.
                             ...
                             if p.team == game.away_team.code:
                                 ...
                                 # Home Trapper: PASS (Home Star Immunity)
                                 # v3.0.9: We realized fading Home Stars in traps is -EV. They control the limited volume.
                                 continue
            ...
            if "Volume" in p.status_note or "Elite" in p.status_note:
                if opponent.avg_sog_against >= 31.0:
                    # v3.0.8 SAFETY GATES
                    # 1. Cold Player Gate: Never take Over 3.5 on a Cold player, even in good matchup.
                    if form.get("SOG_Trend") == "Cold":
                        continue
                    
                    # 2. Efficiency Mismatch Gate:
                    # If we are the Efficiency Favorite (e.g. VAN vs ANA), we don't need volume to score.
                    # Only take Overs for True Elite Volume players (Avg > 2.9) who command share.
                    if is_efficiency_favored and p.avg_sog < 2.9:
                        continue
